...
class CodeGenerator:
    ...
    def _generate_module_def(self, output_dir):
        template = self.template_env.get_template('core/module.def.template')
        for module in self.config.modules:
            output_file = os.path.join(output_dir, f'mmagic_core_{module.name}.def')
    ...
    def generate_core_files(self):
        ...
        self._generate_module_def(agent_core_output_dir)
        self._generate_module_header(agent_core_output_dir)

    def _generate_cli_module_def(self, output_dir):
        template = self.template_env.get_template('cli/cli_module.def.template')
        for module in self.config.modules:
            if not module.cli_support:
                continue
            output_file = os.path.join(output_dir, f'mmagic_cli_{module.name}.def')
            self.write_c(output_file, template, module)
    ...
    def _generate_cli_def(self, output_dir):
        template = self.template_env.get_template('cli/mmagic_cli.def.template')
        output_file = os.path.join(output_dir, 'mmagic_cli.def')
        self.write_c(output_file, template)
    ...
    def _generate_m2m_module_source(self, output_dir):
        ...
        for module in self.config.modules:
            output_file = os.path.join(output_dir, f'mmagic_m2m_{module.name}.c')
            self.write_c(output_file, template, module)
    ...
    def _generate_m2m_def(self, output_dir):
        template = self.template_env.get_template('m2m/mmagic_m2m.def.template')
        output_file = os.path.join(output_dir, 'mmagic_m2m.def')
        self.write_c(output_file, template)
    ...
    def generate_m2m_files(self):
        ...
        self._generate_m2m_def(m2m_output_dir)
