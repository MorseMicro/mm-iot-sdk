 ...
 *
 * To initialize LittleFS, we need to first populate a @c struct @c lfs_config
 * with hardware details like block sizes, number of blocks and minimum read and write
 * sizes.  This structure also requires pointers to functions to perform the block read, write
 * and erase functions.  @c mmosal_shim_fileio.c then retreives this information via a call to
 * @c mmhal_get_littlefs_config() to initialize LittleFS.  LittleFS will directly call the HAL
 * functions provided in the structure to read and write from the hardware.
 *
 ...
 */
int mmhal_littlefs_read(const struct lfs_config *c, lfs_block_t block,
 ...
 * @return       LFS_ERR_OK (0) on success, negative values on error.
 */
int mmhal_littlefs_write(const struct lfs_config *c, lfs_block_t block,
